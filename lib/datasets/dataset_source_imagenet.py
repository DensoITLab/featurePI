...
def load_split(train: bool,
  ...
  ds = tfds.load('imagenet2012', split=split, decoders={
...
class ImagenetCorrupted(dataset_source.DatasetSource):
  ...
  def __init__(self, batch_size: int, image_size: int, corruption: str,
               severity: int, image_level_augmentations: str = 'none'):
    """Instantiates the Imagenet dataset source.
    ...
    self._test_ds = tfds.load(f'imagenet_corrupted/{corruption}_{severity}', data_dir=FLAGS.data_dir, split='test', decoders={
      'image': tfds.decode.SkipDecoding(),
  },).cache()
    self._num_classes = 1000
    self._image_level_augmentations = image_level_augmentations
