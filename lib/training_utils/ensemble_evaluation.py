...
flags.DEFINE_integer('ece_bins', 20,
				  'The number of bins used in calculation of ECE.')
...
def cross_entropy_loss(softmax_logits: jnp.ndarray,
...
def temp_scaling(softmax_logits, labels):
	...
	res = scipy.optimize.minimize(obj_fn, 1, method='nelder-mead', options={'xtol': 1e-3})
	return res.x[0]
...
def eval_on_dataset(params, state, dataset, pmapped_eval_step, n_splits=2, n_runs=5):
	...
	"""
	eval_labels, eval_softmax_logits = eval_forward(params, state, dataset, pmapped_eval_step)
	...
	for _ in range(n_runs):
		for tr_idx, te_idx in KFold(n_splits=n_splits, shuffle=True).split(eval_softmax_logits):
			...
			test_metrics = {
				'error_rate':
				jnp.mean(error_rate_metric(test_softmax_logits, test_labels)),
				'loss':
...
def eval(model, optimizer: flax.optim.Optimizer, state: Any,
