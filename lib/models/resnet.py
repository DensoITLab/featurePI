    ...
    def __call__(self, x):
        ...
        if residual.shape != y.shape:
            if self.projection:
                ...
                residual = jnp.pad(residual[:, ::self.strides[0], ::self.strides[1], :], ((
                    0, 0), (0, 0), (0, 0), (self.filters//4, self.filters//4)), "constant")
...
ResNet20 = partial(ResNet, stage_sizes=[3, 3, 3], block_cls=ResNetBlock, num_filters=16, projection=True)
ResNet32 = partial(ResNet, stage_sizes=[5, 5, 5], block_cls=ResNetBlock, num_filters=16, projection=True)
ResNet44 = partial(ResNet, stage_sizes=[7, 7, 7], block_cls=ResNetBlock, num_filters=16, projection=True)
